...
pnorm_f = rpy2.robjects.r['pnorm']
ro.numpy2ri.activate()
# GPfit = importr('BayesGPfit')

# Import truncnorm from R
...
class SIRTGP_probit():
    def __init__(self, Y, X, X0,
        ...
        self.x_sum_nt = (X ** 2).sum(dim=(0,2))

#        grids = GPfit.GP_generate_grids(d = 1, num_grids = self.T, grids_lim = np.array((-1,1)))
        ...
        print((self.beta * self.thresh_beta).shape)
        self.SR = self.update_SR()
        self.S0 = self.update_S0()

        if init_z is None:
            init_z = torch.randn(self.N) + self.SR + self.S0 + self.beta0
        self.z = init_z
    ...
    def update_z(self):
        z_temp = self.SR + self.S0 + self.beta0
        # self.z[self.y_one] = torch.from_numpy(truncnorm.rvs(0 - z_temp[self.y_one], np.inf, loc=z_temp[self.y_one], scale=1, size=self.one_num))
        # self.z[self.y_zero] = torch.from_numpy(truncnorm.rvs(-np.inf, 0 - z_temp[self.y_zero], loc=z_temp[self.y_zero] , scale=1, size=self.zero_num))
       
        self.z[self.y_one] = torch.from_numpy(r_truncnorm(self.one_num, mean=( z_temp[self.y_one]).numpy(), sd=1, a=0, b=np.inf)).float()
        self.z[self.y_zero] = torch.from_numpy(r_truncnorm(self.zero_num, mean=( z_temp[self.y_zero]).numpy(), sd=1, a=-torch.inf, b=0)).float()

    
    def update_e(self):
        temp = self.z - self.SR - self.S0 - self.beta0
        self.B_lamb_thresh_X = torch.einsum('tl,nkt->nlk', self.B_lamb, self.thresh_beta.t()[None,:,:] * self.X) / self.ns1
        beta_res = self.beta_hat - self.beta
        for k in range(self.K): 
            for l in range(self.L):
                beta_res[:,k] += self.B_lamb[:,l] * self.e[l,k]
                temp += self.B_lamb_thresh_X[:,l,k] * self.e[l,k]
                sig2 = 1 / (torch.sum((self.B_lamb_thresh_X[:,l,k] ** 2)) + 1 / (self.sigma2_e) + torch.sum((self.B_lamb[:,l] ** 2)) / self.xisq)
                mu = sig2 * (torch.sum(self.B_lamb_thresh_X[:,l,k] * temp) + torch.sum(self.B_lamb[:,l] * beta_res[:,k] / self.xisq))
        ...
        if self.w1 > 0:
            ...
            for k in range(self.K):
                for t in range(self.T):
                    ...
                    c_sum = - 0.5 * torch.sum((temp - mu * self.X[:,k,t] / self.ns1) ** 2)
                    p_mp = p_mp_mat[t,k] 
                    p_m = p_m_mat[t,k] 
                    p_mm = p_mm_mat[t,k] 

                    logc_vec[0] = c_sum + p_mp #C-
                    logc_vec[1] = - 0.5 * torch.sum(temp ** 2) + p_m + torch.log1p(-torch.exp(p_mp-p_m) + 1e-10)
            ...
            for k in range(self.K):
                for t in range(self.T):
                    ...
                    self.a, self.b = (lower - mu) / xi, (upper - mu) / xi
                    #print(r_truncnorm(1, mean=mu.item(), sd=xi.item(), a=lower, b=upper))
                    self.beta_hat[t,k] = torch.from_numpy(r_truncnorm(1, mean=mu, sd=xi, a=lower, b=upper)).float()
                    self.thresh_beta[t,k] = self.RTGP_thresh(self.beta_hat[t,k], self.w1)
                    temp -= (mu * self.thresh_beta[t,k] * self.X[:,k,t]) / self.ns1
        self.SR = self.update_SR()
    ...
    def update_eta(self):
        temp = self.z - self.SR - self.beta0 - self.S0
        thresh_eta_X0 = self.thresh_eta[None, :] * self.X0 / self.ns2

        for v in range(self.V): 
            temp += self.eta[v] * thresh_eta_X0[:,v] 
            sigma2_eta = 1 / (torch.sum((thresh_eta_X0[:,v]  ** 2)) + 1/(self.sigma2_eta)+ 1/self.xisq)
            mu_beta = sigma2_eta * (torch.sum(thresh_eta_X0[:,v]  * temp) + self.eta_hat[v] /self.xisq)
            self.eta[v] = torch.randn(1) * sigma2_eta.sqrt() + mu_beta #norm.rvs(mu_beta, torch.sqrt(sigma2_eta), 1)
            temp -= self.eta[v] * thresh_eta_X0[:,v] 
        self.S0 = self.update_S0()
    ...
    def update_eta_hat(self):
        ...
        if self.w2 > 0:
            ...
            for v in range(self.V):
                ...
                c_sum = - 0.5 * torch.sum((temp - mu * self.X0[:,v] / self.ns2) ** 2)
                p_mp = p_mp_mat[v] 
                p_m = p_m_mat[v] 
                p_mm = p_mm_mat[v] 
    ...
    def update_w1(self):
        ...
        temp = self.z - self.S0 - self.beta0
        self.w_log_post = torch.zeros(self.M)
        for ind in range(self.M):
            ...
            self.w_log_post[ind] = - torch.sum(temp - SR_temp)**2 / 2  + self.w_log_prior[ind]
            # mu = SR_temp + self.S0
            # log_cdf = torch.from_numpy(pnorm_f(mu.numpy(), log_p=True))
            # log_1cdf = torch.from_numpy(pnorm_f(mu.numpy(), log_p=True, lower_tail=False))
            # self.w_log_post[ind] = torch.sum(self.y * log_cdf + (1-self.y) * log_1cdf) + self.w_log_prior[ind]
    ...
    def update_beta0(self):
        temp = self.z - self.SR - self.S0 
        sig2 = 1 / (self.N + 1/self.sigma2_beta0)
        mu_beta = sig2 * (torch.sum(temp))
        self.beta0 = torch.randn(1) * sig2.sqrt() + mu_beta 
    ...
    def update_sigma2_e(self):
        ...
        b_eps_new = torch.sum((self.e  ** 2 )) / 2  + 0.0001
        m = torch.distributions.Gamma(a_eps_new, b_eps_new)
    ...
    def update_loglik_y(self):
        ...
        Phi_mu = torch.special.ndtr(mu)  # Compute Φ(mu)
        logll = (self.y * torch.log(Phi_mu + 1e-10) + (1 - self.y) * torch.log(1 - Phi_mu + 1e-10)).sum()
        # log_cdf = pnorm_f(mu, log_p=True)
        # log_1cdf = pnorm_f(mu, log_p=True, lower_tail=False)
        # logll = torch.sum(self.y * log_cdf + (1-self.y) * log_1cdf)
        return logll
    ...
    def post_means(self):
        ...
        post_e = torch.mean(self.mcmc_e, 0)
        ...
        return post_beta, post_eta, post_beta_hat, post_eta_hat, post_thresh_beta, post_thresh_eta, post_e, post_w1, post_w2, post_sigma2_eta, post_sigma2_e, post_xisq, post_beta0
    ...
    def predict_score(self, X_test, X0_test):
        ...
        for i in range(ndraw):
            ...
            p = torch.special.ndtr(z)
            pred_U[i] = torch.bernoulli(p)
            # p = pnorm_f(mu, log_p=False)
            # pred_U[i] = torch.random.binomial(1, p)
